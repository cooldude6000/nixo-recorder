...
const WELCOME_NOTE = `Welcome to Nixo 👋


This note is a quick way to see how Nixo works.


Click **Join & record** in the top-right corner. It will open a private, prerecorded demo meeting, so you don't have to worry about your camera or microphone. Nixo will save the audio. To create a transcript and notes, choose a provider in **Settings → Transcription**; if one is not ready, Nixo will show you a setup shortcut.


When the video ends, Nixo will stop listening. If transcription and intelligence are configured, it will start creating your summary automatically.`;
...
async function findOrCreateWelcomeSession(): Promise<string> {
  ...
  const event: SessionEvent = {
    ...
    calendar_id: "",
    title: "Welcome to Nixo",
    started_at: now,
    ended_at: "",
    is_all_day: false,
    has_recurrence_rules: false,
    meeting_link: WELCOME_NOTE_DEMO_URL,
    description: "A private, prerecorded introduction to Nixo.",
  };

  return createSession("Welcome to Nixo", DEFAULT_USER_ID, {
